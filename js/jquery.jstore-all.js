	...
	$.extend($.jStore, {
		...
		delegate: new jStoreDelegate($.jStore)
			.bind('jStore-ready', function(engine){
                if (debug_jstore) alert('$.jStore.isReady = true;');
				$.jStore.isReady = true;
				if ($.jStore.defaults.autoload) {
                    if (debug_jstore) alert('connect');
                    engine.connect();
                }
			})
			.bind('flash-ready', function(){
                if (debug_jstore) alert('$.jStore.isFlashReady = true;');
		...
		flashReady: function(callback){
            if (debug_jstore) alert('flashReady callback defined');
		...
		use: function(engine, project, identifier){
			...
			if (e.isAvailable()){
                ...
                if (debug_jstore) alert('trigger jStore-ready');
		...
		safeStore: function(value){
			switch (typeof value){
				case 'object': case 'function': return $.toJSON(value);
		...
		init: function(project, name){
			...
			this.delegate = new jStoreDelegate(this)
				.bind('engine-ready', function(){
                    if (debug_jstore) alert('engine.isReady = true;');
					this.isReady = true;
				})
...
(function($){
	...
	this.jStoreDom = StorageEngine.extend({
		...
		connect: function(){
            if (debug_jstore) alert('local connect()');
...
(function($){
	...
	this.jStoreFlash = StorageEngine.extend({
		init: function(project, name){			
			...
			var self = this;
            if (debug_jstore) alert('flashReady function bound to jStore');
			$.jStore.flashReady(function(){ self.flashReady() });
		},
		connect: function(){
			var name = 'jstore-flash-embed-' + this.project;
			if (debug_jstore) alert('flash connect()');
            ...
            // ralf: DOM needs to be ready to append the iframe !!!
            $(document.body)
            ...
            if (debug_jstore) alert('iframe appended');
		},
		flashReady: function(){
            if (debug_jstore) alert('flashReady() called');
			var iFrame = $('#jStoreFlashFrame')[0];
			if (iFrame.Document && $.isFunction(iFrame.Document['jStoreFlash'].f_get_cookie)) {
				// IE
                this.db = iFrame.Document['jStoreFlash'];
			} else if (iFrame.contentWindow && iFrame.contentWindow.document) {
				// Safari && Firefox
                // if (iFrame.contentWindow.document) alert('OK');
				var doc = $(iFrame.contentWindow.document);
                var flash_object = $('object', doc)[0];
                var flash_embed = $('embed', doc)[0];
				if (flash_object && typeof flash_object.f_get_cookie == 'function') {
					// Safari
                    if (debug_jstore) alert("Safari");
                    this.db = flash_object;
				} else if (flash_embed && typeof flash_embed.f_get_cookie == 'function') {
					// Firefox
                    if (debug_jstore) alert("Firefox");
                    this.db = flash_embed;
                }
			}
			// We're ready to process data
            if (debug_jstore) alert('trigger engine-ready event');
...
function flash_ready(){
    if (debug_jstore) alert('trigger flash-ready event');
